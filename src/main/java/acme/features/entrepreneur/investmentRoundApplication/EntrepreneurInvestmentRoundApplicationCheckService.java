...
@Service
public class EntrepreneurInvestmentRoundApplicationCheckService implements AbstractUpdateService<Entrepreneur, InvestmentRoundApplication> {
	...
	public void bind(final Request<InvestmentRoundApplication> request, final InvestmentRoundApplication entity, final Errors errors) {
		...
		assert entity != null;
		assert errors != null;
		request.bind(entity, errors, "ticker", "creationMoment", "statement", "offer", "investmentRound", "pass");
	...
	public void unbind(final Request<InvestmentRoundApplication> request, final InvestmentRoundApplication entity, final Model model) {
		...
		assert entity != null;
		assert model != null;
		request.unbind(entity, model, "link", "checked");
	}
	...
	public void validate(final Request<InvestmentRoundApplication> request, final InvestmentRoundApplication entity, final Errors errors) {
		...
		if (!errors.hasErrors("check")) {
			String pass = this.repository.findPass(entity.getId());
			boolean same = request.getModel().getString("check").equals(pass);
			errors.state(request, same, "check", "entrepreneur.investmentRoundApplication.error.password");
		}
	...
	public void update(final Request<InvestmentRoundApplication> request, final InvestmentRoundApplication entity) {
		...
		if (request.getModel().getString("check").equals(this.repository.findPass(entity.getId()))) {
