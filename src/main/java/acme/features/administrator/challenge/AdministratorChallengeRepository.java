...
public interface AdministratorChallengeRepository extends AbstractRepository {
	...
	@Query("select c from Challenge c where (c.deadlineDate >= CURRENT_TIMESTAMP)")
	Collection<Challenge> findMany();

	@Query("select c from Challenge c where c.deadlineDate > ?1 ")
	Collection<Challenge> findActiveChallenges(Date moment);
}
