...
public class AnonymousUserAccountCreateService implements AbstractCreateService<Anonymous, UserAccount> {
	...
	public void validate(final Request<UserAccount> request, final UserAccount entity, final Errors errors) {
		...
		boolean isDuplicated, isAccepted, isMatching, isEmpty;
		...
		if (!errors.hasErrors("identity.email")) {
			isEmpty = request.getModel().getString("identity.email") == "" || request.getModel().getString("identity.email").isEmpty() || request.getModel().getString("identity.email") == null;
			errors.state(request, !isEmpty, "identity.email", "default.error.email");
		}
	}
